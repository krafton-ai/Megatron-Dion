...
def model_provider(
    ...
    **kwargs,
    ...
    ps_factor = getattr(args, 'pixel_shuffle_factor', 2) if args.pixel_shuffle else False
    ...
    # Pass optional vision-head override into config builder.
    vision_config.encoder_num_attention_heads = getattr(args, "encoder_num_attention_heads", None)
    ...
    if vision_model_type in ["clip", "siglip", "siglip2_base", "radio", "cradio-g"]:
        if use_te:
    ...
    model = LLaVAModel(
        ...
        pixel_shuffle=ps_factor if args.pixel_shuffle else False,
        tile_tags=tile_tags,
