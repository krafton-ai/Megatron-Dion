#!/bin/bash
# =============================================================================
# DION Test: TP=2, PP=2, FS=2 (same settings as run_fineweb_dion_distributed.sh)
# =============================================================================
...
# 12 layers / PP=2 / VPP=2 = 3 layers per virtual stage
USE_VPP=${USE_VPP:-1}
NUM_LAYERS_PER_VPP=${NUM_LAYERS_PER_VPP:-3}
...
# VPP flag (only works with PP > 1)
if [ "$USE_VPP" = "1" ]; then
    VPP_FLAG="--num-layers-per-virtual-pipeline-stage ${NUM_LAYERS_PER_VPP}"
else
    VPP_FLAG=""
fi

# =============================================================================
# Parallelism: TP=2, PP=2, FS=2
# =============================================================================
TP_SIZE=2
PP_SIZE=2
FS_SIZE=2
...
CHECKPOINT_DIR="checkpoints/dion_fs${FS_SIZE}_tp${TP_SIZE}_pp${PP_SIZE}_rank${DION_RANK_FRACTION}"
TENSORBOARD_DIR="tensorboard/dion_fs${FS_SIZE}_tp${TP_SIZE}_pp${PP_SIZE}_rank${DION_RANK_FRACTION}"
...
echo "============================================"
echo "DION: TP=${TP_SIZE}, PP=${PP_SIZE}, FS=${FS_SIZE}"
echo "rank_fraction=${DION_RANK_FRACTION}"
echo "VPP=${USE_VPP} (layers_per_stage=${NUM_LAYERS_PER_VPP})"
echo "SEQ_PARALLEL=${SEQ_PARALLEL}"
