    ...
    def load_common(self, checkpoint_dir: Union[str, Path]):
        ...
        try:
            if MultiStorageClientFeature.is_enabled():
                ...
                # Some checkpoints store custom Python objects in the common
                # state. Since torch 2.6 defaults `torch.load` to
                # `weights_only=True`, explicitly disable it when loading a
                # trusted checkpoint source.
