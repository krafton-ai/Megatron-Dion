...
def _pp_invariant_base_seed() -> int:
    ...
    Megatron training seeds ranks with a PP-dependent offset. Expert parameter
    initialization should not depend on which PP stage owns the layer, so strip
    the PP component here and derive deterministic per-parameter seeds from the
    remaining base seed.
    """
