...
class DionOptimizerConfig(OptimizerConfig):
    ...
    """Momentum factor for error feedback (mu parameter)."""

    dion_rank_fraction: float = 0.25
    """Fraction r/d for low-rank approximation (0.0-1.0)."""

    dion_rank_multiple_of: int = 1
    """Round rank to multiple of this value for efficiency."""

    dion_epsilon: float = 1e-8
    """Epsilon for numerical stability."""

    dion_oversample: float = 1.25
    """Oversampling factor for randomized Cholesky QR."""

    dion_use_fs_collectives: bool = True
    """Enable FS collectives in distributed optimizer mode."""

    dion_use_compressed_comm: bool = False
    """Enable compressed communication (P/R instead of full gradient)."""
    ...
    """Beta2 for scalar optimizer (AdamW/Lion)."""

    dion_eps: float = 1e-8
    """Epsilon for scalar optimizer."""

    dion_momentum_dtype: Optional[torch.dtype] = torch.float32
    """Dtype for momentum state. Recommended: float32 for stability."""
    ...
    """Replicate Parallel (RP) size for 2D parallelism."""
