...
_EYE_CACHE: dict[tuple[str, int | None, torch.dtype, int], Tensor] = {}
...
def _eye(dim: int, *, dtype: torch.dtype, device: torch.device) -> Tensor:
    key = (device.type, device.index, dtype, int(dim))
    cached = _EYE_CACHE.get(key)
    if cached is None or cached.device != device:
        cached = torch.eye(int(dim), dtype=dtype, device=device)
...
def sinkhorn_project(
    ...
    *,
    ...
    for _ in range(int(iters)):
        row_sq = y.float().square().sum(dim=1, keepdim=True)
        _all_reduce_sum_unique_(row_sq, col_groups)
        col_sq = y.float().square().sum(dim=0, keepdim=True)
        y = y / row_sq.clamp_min(float(eps)).sqrt().to(dtype=y.dtype)
        y = y / col_sq.clamp_min(float(eps)).sqrt().to(dtype=y.dtype)
        norm_sq = y.float().square().sum()
        _all_reduce_sum_unique_(norm_sq, col_groups)
        factor = target_numel.sqrt() / norm_sq.sqrt().clamp_min(float(eps))
...
def sinkhorn_project_batched(
    ...
    *,
    ...
    for _ in range(int(iters)):
        y32 = y.float()
        row_sq = y32.square().sum(dim=2, keepdim=True)
        _all_reduce_sum_unique_(row_sq, col_groups)
        col_sq = y32.square().sum(dim=1, keepdim=True)
        y = y / row_sq.clamp_min(float(eps)).sqrt().to(dtype=y.dtype)
        y = y / col_sq.clamp_min(float(eps)).sqrt().to(dtype=y.dtype)
        norm_sq = y.float().square().sum(dim=(1, 2))
        _all_reduce_sum_unique_(norm_sq, col_groups)
        factors = target_numel.sqrt() / norm_sq.sqrt().clamp_min(float(eps))
    ...
    *,
    ...
    if not force_qr:
        try:
            ...
            eye = _eye(int(gram.size(0)), dtype=gram.dtype, device=gram.device)
            chol = torch.linalg.cholesky(gram + float(eps) * eye)
            solved = torch.linalg.solve_triangular(chol, a32.mT, upper=False)
            q_local = solved.mT.to(dtype=a.dtype)
...
def _make_rotation_update(
    *,
    ...
    _all_reduce_sum_unique_(align, column_groups)
    new_rotation = shifted_cholesky_qr(
        align,
        eps=float(config.scqr_eps),
        row_groups=row_groups,
        fallback_groups=column_groups,
...
def _shifted_cholesky_qr_batched(
    ...
    *,
    ...
    _all_reduce_sum_unique_(gram, row_groups)
    eye = _eye(int(gram.size(-1)), dtype=gram.dtype, device=gram.device)
    chol, info = torch.linalg.cholesky_ex(gram + float(eps) * eye)
    failed = bool((info != 0).any().item())
    if failed:
        return None
    solved = torch.linalg.solve_triangular(chol, a32.transpose(-2, -1), upper=False)
    return solved.transpose(-2, -1).to(dtype=a.dtype)
...
def _stack_padded(tensors: Sequence[Tensor], *, rows: int, cols: int) -> Tensor:
    if not tensors:
    ...
    result = tensors[0].new_zeros((len(tensors), rows, cols))
...
def compute_aro_update(
    *,
    ...
    update = unorient_tensor(update_oriented, orientation).contiguous()
    scale = float(getattr(config, "update_rms_scale", 0.2))
    if scale > 0.0:
        stats = torch.stack(
            (
                update.float().square().sum(),
                update.new_tensor(float(update.numel()), dtype=torch.float32),
            )
        )
        _all_reduce_sum_unique_(stats, (*row_groups, *column_groups))
        denom = stats[0].sqrt().clamp_min(float(getattr(config, "scqr_eps", 1e-6)))
        update = update * (scale * stats[1].sqrt() / denom).to(dtype=update.dtype)
...
def compute_aro_updates_batched(
    *,
    ...
    x_batch = _stack_padded(x_rows_list, rows=max_rows, cols=max_cols)
    rotation_batch = _stack_padded(rotations, rows=max_rows, cols=rot_cols)
    ...
    scale = float(getattr(config, "update_rms_scale", 0.2))
    if scale > 0.0:
        stats = torch.empty((len(x_shapes), 2), dtype=torch.float32, device=update_batch.device)
        for index, (rows, cols) in enumerate(x_shapes):
            update_slice = update_batch[index, :rows, :cols]
            stats[index, 0] = update_slice.float().square().sum()
            stats[index, 1] = float(int(rows) * int(cols))
        _all_reduce_sum_unique_(stats, (*row_groups, *column_groups))
        denom = stats[:, 0].sqrt().clamp_min(float(getattr(config, "scqr_eps", 1e-6)))
        factors = (scale * stats[:, 1].sqrt() / denom).to(dtype=update_batch.dtype)
        update_batch = update_batch * factors.view(-1, 1, 1)

    updates: list[Tensor] = []
    new_rotations: list[Tensor] = []
    for index, ((rows, cols), rotation_shape) in enumerate(zip(x_shapes, rotation_shapes)):
        update_oriented = update_batch[index, :rows, :cols].contiguous()
        updates.append(unorient_tensor(update_oriented, orientation).contiguous())
