...
class MegatronDion(Optimizer):
    ...
    def __init__(
        ...
        defaults = dict(
            ...
            retain_step_buffers=bool(retain_step_buffers),
    ...
    def step(self, closure=None):
        ...
        if not bool(self.defaults.get("retain_step_buffers", True)):
    ...
    def _apply_scalar_batches(
        ...
        for scalar_param in scalar_params:
            ...
            scalar_invariant_key = (
                ...
                p.dtype,
                first_moment.dtype,
                _state_tensor_dtype(second_moment, torch.float32),
                float(lr),
                float(weight_decay),
                float(eps),
                float(beta1),
