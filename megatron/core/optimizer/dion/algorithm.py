...
class MegatronDion(Optimizer):
    ...
    def __init__(
        ...
        self._debug_trace_miss = os.getenv("DION_DEBUG_TRACE_MISS", "0") == "1"
        if self._debug_trace_params or self._debug_dump_target_trace:
            logger.info(
                "[DION_DEBUG_TRACE_CONFIG] params=%s steps=%d dump=%s miss=%s tags=%s dump_dir=%s",
                sorted(self._debug_trace_params),
                self._debug_trace_steps,
                self._debug_dump_target_trace,
                self._debug_trace_miss,
                sorted(self._debug_dump_target_tags),
                str(self._debug_trace_dump_dir()),
            )
    ...
    @staticmethod
    def _parse_int_set_env(name: str, default: Optional[set[int]] = None) -> set[int]:
        ...
        for token in value.split(","):
            ...
            try:
                ...
                raise RuntimeError(
                    f"[DION_INVALID_ENV] {name}={value!r} contains non-int token {token!r}"
                ) from exc
            if step <= 0:
                raise RuntimeError(
                    f"[DION_INVALID_ENV] {name} must contain only positive ints, got {step}"
                )
            parsed.add(step)
        return parsed
