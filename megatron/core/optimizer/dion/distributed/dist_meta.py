...
def _linear_split_rows_for_layout(*, model_param, param_name: str, dion_shard_layout, expert_layout):
    if not is_linear_split_param(model_param):
        return None
    split_rows = get_linear_split_rows(model_param)
    if split_rows is None:
        return None
    split_axis = get_linear_split_axis(model_param)

    global_rows = int(dion_shard_layout.global_shape[split_axis])
    ...
    per_expert_rows = int(per_expert_shape[split_axis])
    if int(sum(split_rows)) == per_expert_rows:
        return split_rows

    if expert_layout is not None and int(expert_layout["expert_axis"]) == split_axis:
...
def build_param_dist_meta(
    *,
    ...
    gdn_split_shapes = get_gdn_split_shapes(model_param) if is_gdn_param(model_param) else None
    qkvg_split_axis = get_qkvg_split_axis(model_param) if qkvg_split_shapes is not None else 0
    qkv_split_axis = get_qkv_split_axis(model_param) if qkv_split_shapes is not None else 0
    gdn_split_axis = get_gdn_split_axis(model_param) if gdn_split_shapes is not None else 0
    linear_split_axis = get_linear_split_axis(model_param) if is_linear_split_param(model_param) else 0
    ...
    fused_layouts = [
        name
        for name, shapes in (
            ("QKVG", qkvg_split_shapes),
            ("QKV", qkv_split_shapes),
            ("GDN", gdn_split_shapes),
            ("LINEAR", linear_split_rows),
        )
        if shapes is not None
    ]
    if len(fused_layouts) > 1:
        raise RuntimeError(
            "[DION_FUSED_LAYOUT_AMBIGUOUS] "
            f"param={param_name or id(model_param)} fused_layouts={fused_layouts}"
        )
    fused_global_shape = (
    ...
    if qkvg_split_shapes is not None:
        validate_qkvg_split_shapes_for_rows(
            qkvg_split_shapes,
            rows=int(fused_global_shape[qkvg_split_axis]),
            context=f"param={param_name or id(model_param)}",
        )
    if qkv_split_shapes is not None:
        validate_qkv_split_shapes_for_rows(
            qkv_split_shapes,
            rows=int(fused_global_shape[qkv_split_axis]),
            context=f"param={param_name or id(model_param)}",
        )
    if gdn_split_shapes is not None:
        validate_gdn_split_shapes_for_rows(
            gdn_split_shapes,
            rows=int(fused_global_shape[gdn_split_axis]),
            context=f"param={param_name or id(model_param)}",
        )
    ...
    dist_meta = DionDistMeta(
        ...
        linear_partition_stride=(
            get_linear_partition_stride(model_param)
    ...
    for param_group in param_groups:
        for param in param_group['params']:
            ...
            dist_meta = DionDistMeta(
                ...
                qkv_split_axis=(
                    get_qkv_split_axis(model_param)
                    if is_qkv_param(model_param)
                    else 0
                ),
                qkvg_split_shapes=(
                ...
                qkvg_split_axis=(
                    get_qkvg_split_axis(model_param)
                    if is_qkvg_param(model_param)
                    else 0
                ),
                gdn_split_shapes=(
                    get_gdn_split_shapes(model_param)
                    if is_gdn_param(model_param)
                    else None
                ),
                gdn_split_axis=(
                    get_gdn_split_axis(model_param)
                    if is_gdn_param(model_param)
                    else 0
                ),
