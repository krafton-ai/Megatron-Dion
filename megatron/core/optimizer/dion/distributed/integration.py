...
def _validate_replica_group(
    *,
    ...
    """Validate that the group is already the exact Dion replica domain."""
    if group is None:
        return None
    ...
    if pure_dp_group is not None:
        ...
        leaked_ranks = tuple(rank for rank in group_ranks if rank not in pure_dp_rank_set)
        if leaked_ranks:
            raise RuntimeError(
                "Dion replica group must already be the exact CP-excluded replica domain. "
                f"source={source} requested_rp={requested_rp_world_size} "
                f"group_ranks={group_ranks} pure_dp_ranks={pure_dp_ranks} "
    ...
    *,
    ...
    if overlap != (global_rank,):
        raise RuntimeError(
            ...
            f"source={source} group_ranks={group_ranks} "
...
def _validate_dense_group(
    *,
    ...
    if len(group_ranks) != int(requested_world_size):
        raise RuntimeError(
            "Dion dense group size mismatch. "
            f"source={source} requested_world_size={requested_world_size} "
            f"actual_world_size={len(group_ranks)} group_ranks={group_ranks}"
        )
...
def _validate_expert_fs_group(
    *,
    ...
    """Validate the expert-local FS group from MCore."""
    if group is None:
        if int(requested_world_size) <= 1:
            return None
        raise RuntimeError(
    ...
    if len(group_ranks) != int(requested_world_size):
        raise RuntimeError(
            ...
            f"actual_fs={len(group_ranks)} group_ranks={group_ranks}. "
...
def build_dion_optimizer(
    *,
    ...
    return MegatronDion(
        ...
        split_parameters=config.dion_split_parameters,
