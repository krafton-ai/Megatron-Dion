...
class DistributedDionOptimizer(DistributedMatrixOptimizer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._dion_q_sync_pending = False
    ...
    def _prepare_split_child_fs_groups_for_known_metas(self) -> None:
        ...
        for dist_meta in dist_metas:
            ...
            if split_shapes is not None:
                split_axis = resolve_qkvg_split_axis(dist_meta=dist_meta)
            ...
            if split_shapes is not None:
                split_axis = resolve_qkv_split_axis(dist_meta=dist_meta)
                ...
                continue
            split_shapes = resolve_gdn_split_shapes(
                param=None,
                optimizer_state=None,
                dist_meta=dist_meta,
            )
            if split_shapes is not None:
                split_axis = resolve_gdn_split_axis(dist_meta=dist_meta)
                self._init_gdn_child_groups(
                    dist_meta=dist_meta,
                    split_shapes=split_shapes,
                    split_axis=split_axis,
                )
            ...
            if split_rows is not None:
                split_axis = resolve_linear_split_axis(dist_meta=dist_meta)
                child_kinds = resolve_linear_child_kinds(dist_meta=dist_meta)
    ...
    def _sync_dion_state(self, dion_params: List[Tuple]) -> None:
        ...
        if not bool(getattr(self, "_dion_q_sync_pending", False)):
            return
        sync_q_replicas(
            dion_params=dion_params,
            state_replica_group=self.state_replica_group,
    ...
    def _refresh_dion_param_config(
        ...
        *,
        ...
        r_override = int(r_global_override) if r_global_override is not None else None
        signature = (
            ...
            bool(getattr(self.optimizer, "use_low_rank_sync", False)),
            r_override,
            rank_fraction,
            rank_multiple_of,
        )
        if (
            getattr(dist_meta, "_dion_param_config_signature", None) == signature
        ...
        dist_meta._dion_param_config_signature = signature
    ...
    def _init_optimizer_state(self, param, state, optim_group) -> None:
        ...
        split_gdn_shapes = (
            resolve_gdn_split_shapes(param=param, optimizer_state=state, dist_meta=dist_meta)
            if split_parameters_enabled
            else None
        )
        split_linear_rows = (
        ...
        if is_dion_eligible and split_linear_rows is not None and dist_meta is not None:
            split_axis = resolve_linear_split_axis(
                param=param,
                optimizer_state=state,
                dist_meta=dist_meta,
            )
            child_kinds = resolve_linear_child_kinds(
                param=param,
                optimizer_state=state,
                dist_meta=dist_meta,
            )
        ...
        if is_dion_eligible and split_qkvg_shapes is not None and dist_meta is not None:
            split_axis = resolve_qkvg_split_axis(
                param=param,
                optimizer_state=state,
                dist_meta=dist_meta,
            )
        ...
        if is_dion_eligible and split_qkv_shapes is not None and dist_meta is not None:
            split_axis = resolve_qkv_split_axis(
                param=param,
                optimizer_state=state,
                dist_meta=dist_meta,
            )
        ...
        if is_dion_eligible and split_gdn_shapes is not None and dist_meta is not None:
            split_axis = resolve_gdn_split_axis(
                param=param,
                optimizer_state=state,
                dist_meta=dist_meta,
            )
        ...
        init_param_state(
            ...
            split_parameters_default=split_parameters_enabled,
            q_init=q_init,
        )
        if any(
            str(key).endswith("_needs_state_replica_q_sync") and bool(value)
            for key, value in state.items()
        ):
            self._dion_q_sync_pending = True
    ...
    def _mark_split_child_grad_reduced(self, parent_grad, child_grad) -> None:
        """Preserve RP-reduced-grad cache state across optimizer-only split copies."""
        if parent_grad is None or child_grad is None:
            return
        ...
        if replica_group is None or self._group_size(replica_group) <= 1:
            return
        before_step = int(getattr(self.optimizer, "_step_count", 0)) - 1
        if before_step < 0:
            return
        reduce_op = replicate_reduce_op(self.optimizer)
        state = dense_cache_state(
            self.optimizer,
            parent_grad,
        ...
        if state == "match":
            ...
            raise RuntimeError("[DION_SPLIT_PARENT_RP_GRAD_CACHE_MISMATCH]")

    def _should_use_distributed_dion_update(self, param, state, optim_group, dist_meta) -> bool:
        """Return the distributed Dion route from the adapter."""
    ...
    def _normalize_linear_split_rows_for_meta(
        ...
        if per_expert_shape is None:
            return split_rows
        per_expert_rows = int(per_expert_shape[int(split_axis)])
        if sum(split_rows) == per_expert_rows:
            return split_rows
        parent_global_shape = getattr(dist_meta, "global_shape", None)
        parent_rows = int(parent_global_shape[int(split_axis)]) if parent_global_shape is not None else 0
        expert_axis = int(getattr(dist_meta, "expert_axis", -1))
        num_local_experts = int(getattr(dist_meta, "num_local_experts", 1))
        if (
            expert_axis == int(split_axis)
        ...
        *,
        ...
        return build_split_child_dist_meta(
            ...
            child_fields={
                ...
                "qkvg_split_axis": int(split_axis),
            },
    ...
    def _resolve_qkvg_child_row_group_layout(
        ...
        *,
        ...
        parent_global_rows = int(parent_global_shape[int(split_axis)])
    ...
    def _resolve_qkvg_child_fs_shard_layout(
        ...
        *,
        ...
        if parent_fs_shard_dim != int(split_axis) or parent_fs_world_size <= 1:
            return (
                parent_fs_group,
    ...
    def _resolve_qkvg_child_tp_shard_layout(
        ...
        *,
        ...
        if parent_tp_shard_dim != int(split_axis) or parent_tp_world_size <= 1:
            return (
                parent_tp_group,
    ...
    def _build_gdn_child_dist_meta(
        self,
        *,
        parent_dist_meta: DionDistMeta,
        child_kind: str,
        ...
        """Build optimizer-only child metadata for one fused GDN child."""
        child_uid = gdn_child_param_uid(parent_dist_meta.param_uid, child_kind)
        child_name = gdn_child_name(parent_dist_meta.param_name, child_kind)
        (
            child_fs_group,
            child_fs_world_size,
            child_fs_rank,
            ...
            child_fs_end_idx,
            fs_row_shard_sizes,
        ) = self._resolve_gdn_child_fs_shard_layout(
            parent_dist_meta=parent_dist_meta,
            split_shapes=split_shapes,
            split_axis=split_axis,
            child_kind=child_kind,
        ...
        (
            ...
            tp_row_shard_end_idx,
            tp_row_shard_sizes,
        ) = self._resolve_gdn_child_tp_shard_layout(
            parent_dist_meta=parent_dist_meta,
            split_shapes=split_shapes,
            split_axis=split_axis,
            child_kind=child_kind,
        ...
        return build_split_child_dist_meta(
            ...
            child_fields={
                "is_transposed": False,
                "is_gdn_child": True,
                "gdn_child_kind": child_kind,
                "gdn_split_shapes": tuple(int(dim) for dim in split_shapes),
                "gdn_split_axis": int(split_axis),
            },
    ...
    def _resolve_gdn_child_row_group_layout(
        self,
        *,
        parent_dist_meta: DionDistMeta,
        split_shapes: tuple[int, int, int, int, int, int],
        ...
        """Return child-local ownership for a parent row-sharded GDN group."""
        parent_global_shape = tuple(int(dim) for dim in parent_dist_meta.global_shape)
        child_global_rows = gdn_child_global_shape(
            parent_global_shape,
            split_shapes,
            child_kind,
            split_axis=split_axis,
        ...
        for rank_idx in range(parent_world_size):
            if label == "TP":
                child_range = gdn_child_rank_row_range(
                    split_shapes=split_shapes,
                    child_kind=child_kind,
                ...
                child_range = gdn_child_row_range(
                    parent_row_start=parent_rank_start,
                    parent_row_end=parent_rank_end,
                    split_shapes=split_shapes,
                    child_kind=child_kind,
    ...
    def _resolve_gdn_child_fs_shard_layout(
        ...
        *,
        ...
        """Return child-local FS ownership for one split GDN child."""
        parent_fs_world_size = int(getattr(parent_dist_meta, "fs_world_size", 1))
        parent_fs_rank = int(getattr(parent_dist_meta, "fs_rank", -1))
        parent_fs_shard_dim = int(getattr(parent_dist_meta, "fs_shard_dim", -1))
        parent_fs_group = getattr(parent_dist_meta, "fs_group", None)
        ...
        return self._resolve_gdn_child_row_group_layout(
            parent_dist_meta=parent_dist_meta,
            split_shapes=split_shapes,
            child_kind=child_kind,
            parent_group=parent_fs_group,
    ...
    def _resolve_gdn_child_tp_shard_layout(
        self,
        *,
        parent_dist_meta: DionDistMeta,
        split_shapes: tuple[int, int, int, int, int, int],
        ...
        """Return child-local TP ownership for one split GDN child."""
        parent_tp_world_size = int(getattr(parent_dist_meta, "tp_world_size", 1))
        parent_tp_rank = int(getattr(parent_dist_meta, "tp_rank", -1))
        parent_tp_shard_dim = int(getattr(parent_dist_meta, "tp_shard_dim", -1))
        parent_tp_group = getattr(parent_dist_meta, "tp_group", None)
    ...
    def _ensure_gdn_child_state_(
        ...
        *,
        ...
        """Initialize child-specific Q state for one fused GDN child when needed."""
        child_kind = child_dist_meta.gdn_child_kind
        ...
        if q_layout is None:
            raise RuntimeError(
                "[DION_MISSING_GDN_CHILD_Q_LAYOUT] "
                f"param_uid={child_dist_meta.param_uid} param_name={child_dist_meta.param_name}"
            )
        q_state = init_q_state(
            param=parent_param,
        ...
        if broadcast_q is None:
            raise RuntimeError(
                "[DION_MISSING_GDN_CHILD_BROADCAST] "
                f"param_uid={child_dist_meta.param_uid} param_name={child_dist_meta.param_name}"
            )
        broadcast_q(q_state)

        parent_state[q_key] = q_state
        parent_state[gdn_state_key("r", child_kind)] = int(q_layout.r_global)
        parent_state[gdn_state_key("local_shape", child_kind)] = tuple(
            int(dim) for dim in child_dist_meta.local_shape
        )
        parent_state[gdn_state_key("global_shape", child_kind)] = tuple(
            int(dim) for dim in child_dist_meta.global_shape
        )
        parent_state[f"_gdn_{child_kind}_needs_state_replica_q_sync"] = True
        self._dion_q_sync_pending = True
    ...
    def _build_linear_child_dist_meta(
        ...
        *,
        ...
        child_local_shape: tuple[int, int],
        child_global_shape: tuple[int, int],
        child_kinds: tuple[str, str] = ("gate", "up"),
        split_axis: int = 0,
    ) -> DionDistMeta:
        """Build optimizer-only child metadata for one fused linear child."""
        ...
        return build_split_child_dist_meta(
            ...
            child_fields={
                ...
                "linear_child_kinds": tuple(child_kinds),
                "linear_split_axis": int(split_axis),
    ...
    def _resolve_linear_child_row_group_layout(
        ...
        *,
        ...
        parent_global_shape = tuple(int(dim) for dim in parent_dist_meta.global_shape)
        parent_global_rows = int(parent_global_shape[int(split_axis)])
        child_global_rows = linear_child_global_shape(
            parent_global_shape,
            split_rows,
            child_kind,
            split_axis=split_axis,
    ...
    def _resolve_linear_child_tp_shard_layout(
        ...
        *,
        ...
        if partition_stride == len(split_rows):
            child_kinds = tuple(child_kinds)
            try:
                child_index = child_kinds.index(child_kind)
            except ValueError as exc:
                raise RuntimeError(
                    "[DION_LINEAR_INVALID_CHILD_KIND] "
    ...
    def _init_linear_child_groups(
        ...
        *,
        ...
        split_rows = self._normalize_linear_split_rows_for_meta(
            split_rows,
            dist_meta,
            split_axis=split_axis,
        )
        dist_meta = self._split_parent_dist_meta(dist_meta)
        for child_kind in iter_linear_child_kinds(child_kinds):
            self._resolve_linear_child_fs_shard_layout(
    ...
    def _ensure_linear_child_state_(
        ...
        *,
        ...
        """Initialize child-specific Q state for one fused linear child when needed."""
    ...
    def _expand_split_qkvg_params(
        ...
        *,
        ...
        for child_kind in iter_qkvg_child_kinds():
            ...
            self._mark_split_child_grad_reduced(parent_grad_view, child_grad)
    ...
    def _expand_split_gdn_params(
        ...
        *,
        ...
        if momentum is None:
            raise RuntimeError(
                "[DION_GDN_SPLIT_MISSING_MOMENTUM] "
                f"param_uid={getattr(dist_meta, 'param_uid', None)} "
                f"param_name={getattr(dist_meta, 'param_name', '')}"
            )
        (
        ...
        child_step_params: list[DionStepParam] = []
        for child_kind in iter_gdn_child_kinds():
            if not gdn_child_has_local_overlap(
                split_shapes,
                split_parent_dist_meta,
                child_kind,
                split_axis=split_axis,
                ...
                continue
            child_local_shape = gdn_child_local_shape(
                parent_local_shape,
                split_shapes,
                child_kind,
                dist_meta=split_parent_dist_meta,
            ...
            child_dist_meta = self._build_gdn_child_dist_meta(
                parent_dist_meta=split_parent_dist_meta,
                child_kind=child_kind,
                split_shapes=split_shapes,
                child_local_shape=child_local_shape,
            ...
            self._mark_split_child_grad_reduced(parent_grad_view, child_grad)
            child_momentum = extract_gdn_child(
                parent_momentum_view,
                split_shapes,
                child_kind,
                dist_meta=split_parent_dist_meta,
            ...
            q_key = gdn_state_key("Q", child_kind)
            r_key = gdn_state_key("r", child_kind)
            local_shape_key = gdn_state_key("local_shape", child_kind)
            global_shape_key = gdn_state_key("global_shape", child_kind)
            sync_key = f"_gdn_{child_kind}_needs_state_replica_q_sync"
            self._refresh_dion_param_config(
                dist_meta=child_dist_meta,
                optim_group=optim_group,
                local_shape=tuple(int(dim) for dim in optimizer_state[local_shape_key]),
        ...
        if not child_step_params:
            raise RuntimeError(
                "[DION_GDN_SPLIT_NO_LOCAL_CHILDREN] "
                f"param_uid={getattr(dist_meta, 'param_uid', None)} "
                f"param_name={getattr(dist_meta, 'param_name', '')} "
                f"split_shapes={split_shapes} "
                f"parent_local_shape={parent_local_shape}"
    ...
    def _expand_split_dion_params(
        ...
        *,
        ...
        if qkv_children is not None:
            return qkv_children
        gdn_children = self._expand_split_gdn_params(
            param=param,
            grad=grad,
            optimizer_state=optimizer_state,
            optim_group=optim_group,
    ...
    def _iter_split_child_dist_metas(self, *, param, state, dist_meta):
        ...
        if bool(self.optimizer.defaults.get("split_parameters", False)):
            ...
            gdn_shapes = resolve_gdn_split_shapes(
                param=param,
                optimizer_state=state,
                dist_meta=dist_meta,
            )
            if gdn_shapes is not None:
                split_axis = resolve_gdn_split_axis(
                    param=param,
                    optimizer_state=state,
                    dist_meta=dist_meta,
                ...
                for child_kind in iter_gdn_child_kinds():
                    ...
                    yield (
                        "gdn",
                        self._build_gdn_child_dist_meta(
                            parent_dist_meta=split_parent_dist_meta,
                            child_kind=child_kind,
    ...
    def _log_grad_issue(
        ...
        **extra,
        ...
        logger.error("[DION_GRAD_ISSUE] %s", log_data)

    def _restore_bucket_param_views_(
        self, *, dion_only: bool = False, matrix_only: bool = False
    ) -> None:
        """Make Dion model params alias canonical bucket.param_data before step updates."""
    ...
    def sharded_state_dict(
        ...
        and store only Dion-specific per-param state keyed by
        `param_uid`.
        """
    ...
    def load_state_dict(self, state_dict):
        ...
        restore_summary = restore_distributed_checkpoint_state(
            param_state_data=param_state_data,
            param_groups=self.optimizer.param_groups,
            optimizer_state=self.optimizer.state,
            get_param_key=self._shard_param_uid,
        ...
        logger.info(
            ...
            restore_summary["missing_state_entry"],
            restore_summary["unnamed"],
        )

        # Master params are restored directly from the Dion checkpoint state.
