...
def _profile_desc(batch_group, dist_metas, real_batch_size: int) -> str:
    ...
    elif getattr(meta, "is_gdn_child", False):
        child = f" gdn={getattr(meta, 'gdn_child_kind', '')}"
...
def _stack_tensors_as_dtype(
    ...
    *,
    ...
    if all(tensor.dtype == dtype for tensor in tensors):
        ...
        return torch.stack(tensors, dim=0, out=result)
    if result is None:
    ...
    for collective in batch_collectives.fs_p_collectives:
        ...
        if process_group and collective.world_size > 1:
            ...
            if len(indices) == batch_size and len(batch_collectives.fs_p_collectives) == 1:
                ...
                tensors_for_handle = tensors
            pending.append((handle, tensors_for_handle))
    if pending:
        yield
        for handle, _tensors in pending:
            handle.wait()
    yield
...
def apply_batch_updates(
    ...
    for index in range(real_batch_size):
        ...
        update = delta.to(param.dtype) if delta.dtype != param.dtype else delta
        key = (param.device, param.dtype)
        update_groups.setdefault(key, ([], []))
        update_groups[key][0].append(param)
        update_groups[key][1].append(update)

    for (_device, _dtype), (group_params, group_updates) in update_groups.items():
        if weight_decay > 0:
            torch._foreach_mul_(group_params, 1 - lr * weight_decay)
        torch._foreach_add_(group_params, group_updates, alpha=-scaled_lr)
...
def run_low_rank_sync_async(
    ...
    """Run the low-rank Dion synchronization path from the adapter."""
    if batch_group is None:
        raise RuntimeError(
...
def batch_dion_update_async(
    ...
    if cached_buffer is not None and M_for_matmul:
        ...
        torch.stack(M_for_matmul, dim=0, out=M_batch)
    else:
