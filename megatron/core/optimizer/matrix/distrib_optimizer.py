...
class DistributedMatrixOptimizer(DistributedOptimizer):
    ...
    @staticmethod
    def _free_matrix_overlap_buffers(bucket) -> None:
        clear_bucket_grad_buffers(bucket)
        clear_param_gather_buffers(bucket)
    ...
    def _log_grad_issue(self, kind: str, model_param, shard_param=None, **extra) -> None:
        ...
        logger.error("[MATRIX_GRAD_ISSUE] %s", log_data)
    ...
    def _restore_bucket_param_views_(self, *, matrix_only: bool = False) -> None:
        """Make Matrix model params alias canonical bucket.param_data before updates."""
        if not hasattr(self, "buffers"):
            return
        for buffer in self.buffers:
            for bucket in buffer.buckets:
