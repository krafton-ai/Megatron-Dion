...
class MatrixGradTransport:
    ...
    bucket_grad: torch.Tensor | None = None
...
def _get_grad_route(
    *,
    ...
    rank_numel = matrix_numel + int(standard_numel)
    if rank_numel <= 0:
        raise RuntimeError(
...
def _flat_source_indices_for_entry(entry, *, rank_start: int, rank_end: int, device) -> torch.Tensor:
    ...
    col_offsets = torch.arange(int(rank_start), int(rank_end), dtype=torch.long, device=device).view(
        1, int(rank_end) - int(rank_start)
    )
    return (base + row_offsets + col_offsets).reshape(-1)


def _build_grad_reduce_indices(
    *,
    bucket,
    matrix_layout,
    group_size: int,
    ...
    device = bucket.grad_data.device
    for group_rank in range(int(group_size)):
        group_rank_start = int(group_rank) * int(rank_numel)
        for entry in matrix_layout.entries:
            ...
            source_indices = _flat_source_indices_for_entry(
                entry,
                rank_start=int(rank_start),
                rank_end=int(rank_end),
                device=device,
            )
            source_numel = int(source_indices.numel())
            ...
            if source_numel > 0:
                ...
                target_parts.append(
                    torch.arange(
                        target_start,
                        target_start + source_numel,
                        dtype=torch.long,
                        device=device,
                    )
                )
                source_parts.append(source_indices)

        if int(standard_numel) > 0:
            standard_start = group_rank_start + int(matrix_numel)
            standard_end = standard_start + int(standard_numel)
            for source_start, source_end, target_start, _ in standard_rank_segments[group_rank]:
                ...
                if target_end > standard_end:
                    raise RuntimeError(
                        "[Matrix] standard grad route exceeds rank buffer "
                        f"bucket={getattr(bucket, 'bucket_id', -1)} "
                        f"target_end={target_end} rank_end={standard_end}"
                    )
                if source_end > source_start:
                    source_parts.append(
...
def _build_grad_reduce_input(
    *,
    ...
    expected_numel = int(route.group_size) * int(route.rank_numel)
    if (
        reduce_input.ndim != 1
    ...
    target_indices = route.reduce_target_indices
    source_indices = route.reduce_source_indices
    if target_indices is None or source_indices is None:
        return reduce_input
    if int(target_indices.numel()) != int(source_indices.numel()):
        raise RuntimeError(
            "[Matrix] Matrix grad reduce index size mismatch "
            f"bucket={getattr(bucket, 'bucket_id', -1)} "
            f"target_numel={int(target_indices.numel())} "
            f"source_numel={int(source_indices.numel())}"
        )
    source_grads = _get_bucket_grad_buffer(
        bucket,
        cache_attr="_matrix_grad_source_cache",
        shape=(int(source_indices.numel()),),
        dtype=bucket.grad_data.dtype,
        device=bucket.grad_data.device,
    )
    torch.index_select(bucket.grad_data, 0, source_indices, out=source_grads)
    reduce_input.index_copy_(0, target_indices, source_grads)
...
def install_optimizer_grads_(
    *,
    ...
    """Set model-side standard and Matrix grads onto optimizer shard params."""
    for model_group, shard_param_group in zip(model_groups, shard_groups):
        if len(model_group) != len(shard_param_group):
            raise RuntimeError(
                "[Matrix] grad copy requires equal model/shard group lengths "
                f"model_len={len(model_group)} shard_len={len(shard_param_group)}"
            )
        for model_param, shard_param in zip(model_group, shard_param_group):
    ...
    if bucket.has_standard_params:
        if use_distributed_optimizer:
            ...
            for source_start, source_end, local_start in route.standard_local_segments:
                ...
                standard_views.append(local_data_view[local_start:local_end])
            if standard_views:
                torch._foreach_mul_(standard_views, scaling_factor)
        else:
            matrix_param_ids = bucket.matrix_param_ids
            standard_views = []
            for param in bucket.params_list:
                ...
                standard_views.append(bucket.grad_data[int(start) : int(end)])
            if standard_views:
                torch._foreach_mul_(standard_views, scaling_factor)
...
def wait_grad_transport(bucket) -> MatrixGradTransport | None:
    """Ensure one bucket's Matrix reduced grad is ready before any consumer reads it."""
...
def clear_bucket_grad_buffers(bucket) -> None:
    """Drop cached Matrix grad transport tensors for one bucket."""
    clear_grad_transport(bucket)
    for attr in (
        "_matrix_grad_bucket_cache",
        "_matrix_grad_reduce_input_cache",
        "_matrix_grad_source_cache",
    ):
...
def start_matrix_grad_sync(
    ...
    *,
    ...
    bucket_grad = _get_bucket_grad_buffer(
        bucket,
        cache_attr="_matrix_grad_bucket_cache",
        shape=(int(route.rank_numel),),
        dtype=bucket.grad_data.dtype,
        device=bucket.grad_data.device,
    )
    matrix_grad_shard = bucket_grad[: int(route.matrix_numel)]
    standard_grad = None
