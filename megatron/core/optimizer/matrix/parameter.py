"""Matrix parameter-path helpers for the distributed optimizer."""
...
class MatrixParamGatherRoute:
    ...
    input_source_indices: torch.Tensor | None = None
    restore_target_indices: torch.Tensor | None = None
    restore_source_indices: torch.Tensor | None = None
...
def _param_gather_cache(bucket) -> tuple[dict, dict]:
    cache = getattr(bucket, "_matrix_param_gather_buffers", None)
    if cache is None:
        cache = {}
        bucket._matrix_param_gather_buffers = cache
    refs = getattr(bucket, "_matrix_param_gather_buffer_refs", None)
    if refs is None:
        refs = {}
        bucket._matrix_param_gather_buffer_refs = refs
    return cache, refs
...
def clear_param_gather_buffers(bucket) -> None:
    """Drop cached Matrix param all-gather tensors for one bucket."""
    if hasattr(bucket, "_matrix_param_gather_buffers"):
        delattr(bucket, "_matrix_param_gather_buffers")
    if hasattr(bucket, "_matrix_param_gather_buffer_refs"):
        delattr(bucket, "_matrix_param_gather_buffer_refs")
    if hasattr(bucket, "_matrix_param_source_cache"):
        delattr(bucket, "_matrix_param_source_cache")


def _param_source_buffer(
    bucket,
    *,
    name: str,
    shape,
    ...
    cache = getattr(bucket, "_matrix_param_source_cache", None)
    if cache is None:
        cache = {}
        bucket._matrix_param_source_cache = cache
    key = (str(name), tuple(int(dim) for dim in shape), dtype, _param_gather_device_key(device))
    tensor = cache.get(key)
    if tensor is None:
        tensor = torch.empty(tuple(int(dim) for dim in shape), dtype=dtype, device=device)
        cache[key] = tensor
    return tensor
...
def resolve_grad_rank_to_fs_rank(
    *,
    ...
    """Return the FS rank for each grad reduce-scatter output rank."""
    fs_size = int(fs_size)
    if fs_size <= 0:
        raise RuntimeError(f"[Matrix] invalid FS size for bucket={bucket_id}: {fs_size}")

    grad_group_ranks = _group_ranks(grad_group)
    grad_group_size = len(grad_group_ranks)
    if grad_group_size <= 0:
        raise RuntimeError(f"[Matrix] empty grad sync group for bucket={bucket_id}")
    if grad_group_size % fs_size != 0:
        raise RuntimeError(
            "[Matrix] grad sync group is incompatible with FS topology "
    ...
    if fs_group is None:
        raise RuntimeError(
            "[Matrix] missing FS group for multi-rank Matrix grad sync "
    ...
    if len(fs_group_ranks) != fs_size:
        raise RuntimeError(
            "[Matrix] FS group size mismatch for Matrix grad sync "
    ...
    if missing_fs_ranks:
        raise RuntimeError(
            "[Matrix] FS group must be contained in the grad sync group "
    ...
    if len(set(fs_positions)) != fs_size:
        raise RuntimeError(
            "[Matrix] duplicate FS rank position in Matrix grad sync group "
    ...
    if fs_stride <= 0:
        raise RuntimeError(
            "[Matrix] cannot prove FS rank order from authoritative grad/FS groups "
            f"bucket={bucket_id} fs_group_ranks={fs_group_ranks} "
            f"grad_group_ranks={grad_group_ranks} fs_positions={fs_positions} "
    ...
    if any(fs_rank < 0 for fs_rank in grad_rank_to_fs_rank):
        raise RuntimeError(
            "[Matrix] incomplete FS rank mapping for Matrix grad sync "
...
def build_matrix_shard_entries(
    *,
    bucket,
    param_map,
    ...
    """Build bucket-local Matrix shard layouts without mutating parent `param_map`."""
    if grad_shard_group_size <= 0:
        raise RuntimeError(
            f"[Matrix] invalid Matrix grad shard group size: {grad_shard_group_size}"
        )
    if grad_shard_group_size % fs_size != 0:
        raise RuntimeError(
            "[Matrix] Matrix grad shard group is incompatible with FS topology "
            f"(grad_group={grad_shard_group_size}, fs_size={fs_size})"
        )
    if len(grad_rank_to_fs_rank) != int(grad_shard_group_size):
        raise RuntimeError(
            "[Matrix] Matrix grad rank mapping size mismatch "
    ...
    for param in param_map.keys():
        ...
        static_info = matrix_info_by_param.get(param)
        ...
        if not hasattr(bucket, "param_to_index") or param not in bucket.param_to_index:
            raise RuntimeError(
                "[Matrix] missing canonical bucket span for Matrix param "
                f"param={getattr(param, '_param_name', f'id_{id(param)}')}"
            )
        ...
        matrix_shard_layout_by_param[param] = shard_layout
        entries.append(
            MatrixShardEntry(
                param=param,
    ...
    return bucket_layout, matrix_shard_layout_by_param, len(entries)
...
def collect_matrix_bucket_params(matrix_layout: MatrixBucketLayout | None) -> list[torch.nn.Parameter]:
    """Return unique bucket-local Matrix params in canonical entry order."""
...
def serialize_bucket_gather_layout(matrix_layout: MatrixBucketLayout) -> tuple[int, ...]:
    """Serialize bucket-global Matrix gather invariants for cross-rank validation."""
    layout_data: list[int] = [
        int(matrix_layout.entry_count),
        int(matrix_layout.shard_size),
        int(matrix_layout.gathered_numel),
    ...
    for entry in matrix_layout.entries:
        ...
        layout_data.append(len(entry.canonical_rank_flat_segments))
        for rank_segments in entry.canonical_rank_flat_segments:
            layout_data.append(len(rank_segments))
            for seg_start, seg_end in rank_segments:
                layout_data.extend([int(seg_start), int(seg_end)])
    return tuple(layout_data)
...
def _all_gather_layout_tuples(local_layout: tuple[int, ...], group, group_size: int) -> list[tuple[int, ...]]:
    ...
    layout_tensor = torch.zeros(max_len, dtype=torch.int64, device=device)
    if local_layout:
        layout_tensor[: len(local_layout)] = torch.tensor(local_layout, dtype=torch.int64, device=device)
    gathered_layout_tensors = [torch.empty_like(layout_tensor) for _ in range(int(group_size))]
    dist.all_gather(gathered_layout_tensors, layout_tensor, group=group)
    return [
        tuple(int(value) for value in gathered_layout[:length].cpu().tolist())
        for gathered_layout, length in zip(gathered_layout_tensors, lengths)
    ]
...
def validate_bucket_gather_layout(
    *,
    bucket,
    matrix_layout: MatrixBucketLayout,
    shard_group,
) -> None:
    """Validate that all shard ranks agree on one Matrix bucket gather layout."""
    ...
    if mismatched_ranks:
        raise RuntimeError(
            "[MATRIX_BUCKET_GATHER_LAYOUT_MISMATCH] "
...
def write_matrix_shard_(
    *,
    ...
    """Set one Matrix param's canonical local shard view from a full bucket view."""
    ...
    if local_numel != local_target.numel():
        raise RuntimeError(
            "[Matrix] local restore shard size mismatch "
...
def restore_matrix_shards_from_bucket_(
    *,
    matrix_layout: MatrixBucketLayout | None,
    get_full_view_2d: Callable[[object], torch.Tensor],
    update_data_shard: Callable[[torch.nn.Parameter, torch.Tensor], None],
    param_name: Callable[[torch.nn.Parameter], str],
) -> None:
    """Restore all Matrix local shard aliases from canonical bucket.param_data views."""
    if matrix_layout is None or not matrix_layout.has_params:
        return
    for entry in matrix_layout.entries:
        write_matrix_shard_(
...
def _restore_matrix_bucket(
    *,
    ...
    """Restore canonical bucket storage from one gathered Matrix shard buffer."""
    if gathered_buffer.dim() != 2:
        raise RuntimeError(
            f"[Matrix] gathered Matrix bucket buffer must be 2D, got shape={tuple(gathered_buffer.shape)}"
        )
    if gathered_buffer.size(0) != shard_group_size:
        raise RuntimeError(
            "[Matrix] gathered Matrix bucket buffer group-size mismatch "
    ...
    target_indices = route.restore_target_indices
    source_indices = route.restore_source_indices
    if target_indices is not None and source_indices is not None:
        if int(target_indices.numel()) != int(source_indices.numel()):
            raise RuntimeError(
                "[Matrix] Matrix param restore index size mismatch "
                f"bucket={getattr(bucket, 'bucket_id', -1)} "
                f"target_numel={int(target_indices.numel())} "
                f"source_numel={int(source_indices.numel())}"
            )
        source_params = _param_source_buffer(
            bucket,
            name="matrix_restore",
            shape=(int(source_indices.numel()),),
            dtype=gathered_buffer.dtype,
            device=gathered_buffer.device,
        )
        torch.index_select(gathered_buffer.reshape(-1), 0, source_indices, out=source_params)
        bucket.param_data.view(-1).index_copy_(0, target_indices, source_params)
...
def assert_shard_aliased(
    *,
    ...
    for group_index, (model_group, shard_param_group) in enumerate(
        ...
        for param_index, (_, shard_param) in enumerate(zip(model_group, shard_param_group)):
            ...
            if opt_param is None:
                if mismatch_count < 5:
                    logger.error(
                        "[IDENTITY MISMATCH] main_shard_groups[%s][%s] (shape=%s, ptr=%s) NOT FOUND in optimizer.param_groups! Matrix updates will be LOST!",
                        group_index,
                        param_index,
                        shard_param.shape,
                        shard_ptr,
                ...
                if mismatch_count < 5:
                    logger.error(
                        "[Matrix] main_shard_groups[%s][%s] object mismatch: same data_ptr=%s but different object. id(shard)=%s, id(opt)=%s",
                        group_index,
                        param_index,
                        shard_ptr,
                        id(shard_param),
    ...
    if mismatch_count > 0:
        raise RuntimeError(
            "[Matrix] main_shard_groups identity mismatch with optimizer.param_groups: "
...
def build_bucket_param_map(
    ...
    if bucket_size % dp_world_size != 0:
        raise RuntimeError(
            f"[Matrix] bucket_size must be divisible by dp_size for canonical param_map "
...
def mark_matrix_bucket_params(
    ...
    *,
    ...
    fs_size = int(fs_size)
    if fs_size <= 0:
        raise RuntimeError(f"[Matrix] invalid FS size while marking bucket params: {fs_size}")
    ...
    return matrix_param_count, matrix_info_by_param
...
def bucket_full_param_view_2d(bucket, param, entry) -> Optional[torch.Tensor]:
    """Return the canonical full 2D bucket view for one Matrix param."""
    if bucket is None or getattr(bucket, "param_data", None) is None:
        return None
    if not hasattr(bucket, "param_to_index") or param not in bucket.param_to_index:
        return None
    ...
    if full_flat.numel() != expected_numel:
        raise RuntimeError(
            "[Matrix] canonical full-param view size mismatch "
...
def _bucket_param_alias(bucket, param):
    if bucket is None or getattr(bucket, "param_data", None) is None:
        return None
    if not hasattr(bucket, "param_to_index") or param not in bucket.param_to_index:
        return None
    start, _end = bucket.param_to_index[param]
    flat = bucket.param_data.view(-1)
    return tuple(param.data.shape), int(flat.data_ptr()) + int(start) * int(flat.element_size())
...
def set_bucket_param_views(
    ...
    *,
    ...
    """Ensure bucket params alias the bucket's canonical param buffer."""
    if bucket is None or getattr(bucket, "param_data", None) is None:
        return
    ...
    for param in params_to_set:
        ...
        if expected_view is not None:
            param_views.append((param, expected_view))
    source_copies = {}
    if copy_data:
        for param, expected_view in param_views:
            if (
...
def check_bucket_param_views(
    ...
    *,
    ...
    if getattr(bucket, "param_data", None) is None:
        raise RuntimeError(
            f"[Matrix] {context}: bucket {getattr(bucket, 'bucket_id', -1)} missing bucket.param_data"
        )
    params_to_check = list(bucket.params) if params is None else list(params)
    for param in params_to_check:
        expected_alias = _bucket_param_alias(bucket, param)
        if expected_alias is None:
            continue
        expected_shape, expected_data_ptr = expected_alias
        if tuple(param.data.shape) != expected_shape or int(param.data.data_ptr()) != int(expected_data_ptr):
            ...
            logger.error(
                "[MATRIX_PARAM_BUFFER_VIEW_MISMATCH] context=%s bucket_id=%s param=%s param_ptr=%s bucket_ptr=%s",
                context,
            ...
            raise RuntimeError(
                f"[Matrix] {context}: param.data no longer aliases bucket.param_data for {param_name}"
            )


def attach_matrix_bucket_layout_(
    optimizer,
    *,
    gbuf_idx: int,
    buffer,
    ...
    """Validate that stored Matrix entries already target the current runtime bucket."""
    if not hasattr(bucket, "param_to_index") or bucket.param_to_index is None:
        raise RuntimeError(
            f"[Matrix] buffer={gbuf_idx} bucket={bucket.bucket_id} missing bucket.param_to_index"
        )
    ...
    for entry in matrix_layout.entries:
        ...
        if entry_param not in bucket.param_to_index:
            ...
            raise RuntimeError(
                f"[Matrix] buffer={gbuf_idx} bucket={bucket.bucket_id} stored param "
...
def init_matrix_bucket(
    ...
    *,
    ...
    """Configure one bucket that contains at least one Matrix param."""
    attach_matrix_bucket_layout_(
        optimizer,
        gbuf_idx=gbuf_idx,
        buffer=buffer,
        bucket=bucket,
    ...
    if env_flag("MEGATRON_MATRIX_VALIDATE_LAYOUT", default=False):
    ...
    bucket.matrix_optimizer = optimizer
...
def init_standard_bucket(optimizer, *, gbuf_idx: int, buffer, bucket, fs_group) -> None:
    """Configure one bucket that has no Matrix layout."""
    has_matrix = any(getattr(param, "is_matrix_param", False) for param in bucket.params)
    if has_matrix:
        name_map = getattr(optimizer, "param_to_name", {}) or getattr(buffer, "param_to_name", {})
        matrix_params = []
        for param in bucket.params:
            if getattr(param, "is_matrix_param", False):
                param_name = optimizer._lookup_param_name(name_map, param)
                matrix_params.append((id(param), param_name or f"id_{id(param)}", tuple(param.shape)))
        logger.error(
            f"[Matrix] Buffer {gbuf_idx} Bucket {bucket.bucket_id} has Matrix params but no Matrix layout. params={matrix_params}"
        )
        raise RuntimeError(
            f"[Matrix] Buffer {gbuf_idx} Bucket {bucket.bucket_id} has Matrix params but no Matrix layout"
        )
...
def bucket_matrix_param_view(optimizer, bucket, entry: MatrixShardEntry) -> torch.Tensor:
    """Return the canonical full-param view for one Matrix entry."""
    full_view_2d = bucket_full_param_view_2d(bucket, entry.param, entry)
    if full_view_2d is None:
        raise RuntimeError(
            "[Matrix] canonical FS gather requires bucket.param_data view "
...
def _flat_source_indices_for_entry(
    ...
    *,
    ...
    rows, cols = (int(dim) for dim in tuple(entry.param.shape))
    base = int(entry.canonical_bucket_start)
    if int(rank_end) <= int(rank_start):
        return torch.empty((0,), dtype=torch.long, device=device)
    if int(entry.fs_shard_dim) == 0:
        start = base + int(rank_start) * cols
        end = base + int(rank_end) * cols
        return torch.arange(start, end, dtype=torch.long, device=device)
    row_offsets = torch.arange(rows, dtype=torch.long, device=device).view(rows, 1) * cols
    col_offsets = torch.arange(
        int(rank_start),
        int(rank_end),
        dtype=torch.long,
        device=device,
    ).view(1, int(rank_end) - int(rank_start))
    return (base + row_offsets + col_offsets).reshape(-1)


def _cat_index_parts(parts: list[torch.Tensor]) -> torch.Tensor | None:
    if not parts:
        return None
    if len(parts) == 1:
        return parts[0]
...
def _index_route_device(*indices: torch.Tensor | None) -> torch.device | None:
...
def _get_matrix_param_gather_route(
    *,
    bucket,
    matrix_layout: MatrixBucketLayout,
    shard_group_size: int,
    ...
    input_target_parts: list[torch.Tensor] = []
    input_source_parts: list[torch.Tensor] = []
    restore_target_parts: list[torch.Tensor] = []
    restore_source_parts: list[torch.Tensor] = []

    shard_size = int(matrix_layout.shard_size)
    for entry in matrix_layout.entries:
        local_source = _flat_source_indices_for_entry(
            entry,
            rank_start=int(entry.start_idx),
            rank_end=int(entry.end_idx),
            device=device,
        )
        local_numel = int(local_source.numel())
        if local_numel != int(entry.local_numel):
            raise RuntimeError(
                "[Matrix] Matrix param gather local index size mismatch "
                f"bucket={getattr(bucket, 'bucket_id', -1)} "
                f"param={getattr(entry.param, '_param_name', f'id_{id(entry.param)}')} "
                f"indices={local_numel} expected={int(entry.local_numel)}"
            )
        if local_numel > 0:
            target_start = int(entry.shard_offset)
            input_target_parts.append(
                torch.arange(
                    target_start,
                    target_start + local_numel,
                    dtype=torch.long,
                    device=device,
                )
            )
            input_source_parts.append(local_source)

        if len(entry.rank_split_ranges) != int(shard_group_size):
            raise RuntimeError(
                "[Matrix] Matrix param gather rank mapping size mismatch "
                f"bucket={getattr(bucket, 'bucket_id', -1)} "
                f"param={getattr(entry.param, '_param_name', f'id_{id(entry.param)}')} "
                f"group_size={int(shard_group_size)} "
                f"mapping_size={len(entry.rank_split_ranges)}"
            )
        for group_rank, (rank_start, rank_end) in enumerate(entry.rank_split_ranges):
            target_indices = _flat_source_indices_for_entry(
                entry,
                rank_start=int(rank_start),
                rank_end=int(rank_end),
                device=device,
            )
            source_numel = int(target_indices.numel())
            if source_numel > int(entry.shard_capacity):
                raise RuntimeError(
                    "[Matrix] Matrix param restore source exceeds shard capacity "
                    f"bucket={getattr(bucket, 'bucket_id', -1)} "
                    f"param={getattr(entry.param, '_param_name', f'id_{id(entry.param)}')} "
                    f"source_numel={source_numel} shard_capacity={int(entry.shard_capacity)}"
                )
            if source_numel <= 0:
                continue
            source_start = int(group_rank) * shard_size + int(entry.shard_offset)
            restore_source_parts.append(
                torch.arange(
                    source_start,
                    source_start + source_numel,
                    dtype=torch.long,
                    device=device,
                )
            )
    ...
    route = MatrixParamGatherRoute(
        ...
        input_source_indices=_cat_index_parts(input_source_parts),
        restore_target_indices=_cat_index_parts(restore_target_parts),
...
def _refresh_matrix_shard_aliases_(
    *,
    optimizer,
    bucket,
    matrix_layout: MatrixBucketLayout | None,
...
def fill_matrix_shard_buffer(
    ...
    *,
    ...
    """Copy one Matrix local shard into a gather input buffer."""
    ...
    if registered_data_shard is not None:
        if registered_data_shard.numel() != canonical_local_source.numel():
            raise RuntimeError(
                "[Matrix] canonical FS gather source size mismatch "
        ...
        if registered_data_shard.data_ptr() != canonical_local_source.data_ptr():
            raise RuntimeError(
                "[Matrix] canonical FS gather source mismatch "
                f"for param={optimizer._param_name(entry.param) or f'id_{id(entry.param)}'}: "
    ...
    if canonical_local_source.numel() != local_numel:
        raise RuntimeError(
            "[Matrix] local gather source size mismatch "
            f"param={optimizer._param_name(entry.param) or f'id_{id(entry.param)}'} "
...
def prepare_matrix_param_gather(
    optimizer, bucket, *, shard_group_size: int | None = None
) -> Tuple[torch.Tensor, List[Tuple[MatrixShardEntry, torch.Tensor]]]:
    """Build one bucket-local Matrix shard buffer in canonical entry order."""
    matrix_layout = getattr(bucket, "matrix_layout", None)
    if matrix_layout is None or not matrix_layout.has_params:
        return (
            torch.empty(
    ...
    if shard_group_size is None:
        shard_group = getattr(bucket, "matrix_shard_group", None)
        if shard_group is None:
    ...
    prepared_buffer = _acquire_param_gather_tensor(
        bucket,
        name="matrix_input",
        shape=(int(matrix_layout.shard_size),),
        dtype=bucket.param_data.dtype,
        device=bucket.param_data.device,
        route_key=(
            int(matrix_layout.entry_count),
    ...
    if target_indices is not None and source_indices is not None:
        if int(target_indices.numel()) != int(source_indices.numel()):
            raise RuntimeError(
                "[Matrix] Matrix param gather index size mismatch "
                f"bucket={getattr(bucket, 'bucket_id', -1)} "
                f"target_numel={int(target_indices.numel())} "
                f"source_numel={int(source_indices.numel())}"
            )
        ...
        torch.index_select(bucket.param_data.view(-1), 0, source_indices, out=source_params)
        prepared_buffer.index_copy_(0, target_indices, source_params)
...
def restore_bucket_param_data_(
    ...
    *,
    ...
    """Restore canonical bucket.param_data from one bucket-wise gathered Matrix shard buffer."""
    _restore_matrix_bucket(
        bucket=bucket,
...
def _build_standard_gather_indices(
    *,
    bucket,
    group_rank: int,
    standard_numel: int,
    ...
    for source_start, source_end, target_start in rank_segments[int(group_rank)]:
        ...
        input_source_parts.append(
            torch.arange(source_start, source_end, dtype=torch.long, device=device)
        )
        input_target_parts.append(
            torch.arange(target_start, target_end, dtype=torch.long, device=device)
        )

    for rank_i, segments in enumerate(rank_segments):
        rank_base = int(rank_i) * int(standard_numel)
        for source_start, source_end, target_start in segments:
            ...
            restore_target_parts.append(
                torch.arange(source_start, source_end, dtype=torch.long, device=device)
            )
            restore_source_parts.append(
                torch.arange(
                    rank_base + target_start,
                    rank_base + target_start + length,
                    dtype=torch.long,
                    device=device,
                )
            )
    ...
    return (
        _cat_index_parts(input_target_parts),
        _cat_index_parts(input_source_parts),
        _cat_index_parts(restore_target_parts),
        _cat_index_parts(restore_source_parts),
    )
...
def _get_standard_param_gather_route(
    *,
    ...
    if (
        ...
        and _index_route_device(cached.input_target_indices, cached.restore_target_indices)
        in (None, bucket.param_data.device)
    ):
        return cached
    ...
    (
        ...
        restore_target_indices,
        restore_source_indices,
    ) = _build_standard_gather_indices(
        bucket=bucket,
    ...
    route = MatrixStandardGatherRoute(
        ...
        standard_numel=int(standard_numel),
        rank_segments=rank_segments,
        input_target_indices=input_target_indices,
        input_source_indices=input_source_indices,
        restore_target_indices=restore_target_indices,
...
def _standard_param_gather_route(optimizer, bucket):
    matrix_layout = getattr(bucket, "matrix_layout", None)
    if matrix_layout is None or not bucket.has_standard_params:
        return None, None, None

    dp_group, dp_size, dp_rank = get_bucket_shard_group(optimizer, bucket)
    if dp_group is None:
        raise RuntimeError(
            "[Matrix] mixed standard all-gather requires the standard local-shard group."
        )
    if dp_size == 1:
    ...
    if bucket.param_data.numel() % dp_size != 0:
        raise RuntimeError(
            "[Matrix] mixed standard all-gather requires a padded standard bucket "
            f"bucket={getattr(bucket, 'bucket_id', -1)} "
...
def fill_standard_param_gather(
    ...
    target_indices = route.input_target_indices
    source_indices = route.input_source_indices
    if target_indices is None or source_indices is None:
        return
    if int(target_indices.numel()) != int(source_indices.numel()):
        raise RuntimeError(
            "[Matrix] standard param gather index size mismatch "
            f"bucket={getattr(bucket, 'bucket_id', -1)} "
            f"target_numel={int(target_indices.numel())} "
            f"source_numel={int(source_indices.numel())}"
        )
    ...
    torch.index_select(bucket.param_data.view(-1), 0, source_indices, out=source_params)
    target.index_copy_(0, target_indices, source_params)
...
def restore_standard_param_gather_(bucket, gathered_buffer: torch.Tensor, route) -> None:
    ...
    if gathered_buffer.dim() == 1:
        ...
        raise RuntimeError(
            "[Matrix] mixed standard all-gather output shape mismatch "
            f"bucket={getattr(bucket, 'bucket_id', -1)} "
    ...
    if int(target_indices.numel()) != int(source_indices.numel()):
        raise RuntimeError(
            "[Matrix] standard param restore index size mismatch "
            f"bucket={getattr(bucket, 'bucket_id', -1)} "
            f"target_numel={int(target_indices.numel())} "
            f"source_numel={int(source_indices.numel())}"
        )
    source_params = _param_source_buffer(
        bucket,
        name="standard_restore",
        shape=(int(source_indices.numel()),),
        dtype=gathered_view.dtype,
        device=gathered_view.device,
    )
    torch.index_select(gathered_view.reshape(-1), 0, source_indices, out=source_params)
    bucket.param_data.view(-1).index_copy_(0, target_indices, source_params)
...
def all_gather_matrix_params_(
    optimizer,
    bucket,
    async_op=False,
    prepared_gather=None,
):
    """Gather one bucket-local Matrix shard buffer back into canonical bucket.param_data."""
    matrix_layout = getattr(bucket, "matrix_layout", None)
    if matrix_layout is None or not matrix_layout.has_params:
        return None
    ...
    bucket_shard_size = int(matrix_layout.shard_size)
    expected_gathered_numel = int(matrix_layout.gathered_numel)
    if expected_gathered_numel != bucket_shard_size * shard_group_size:
        _release_param_gather_tensor(bucket, prepared_buffer)
        raise RuntimeError(
            "[Matrix] FS gather shard size invariant mismatch "
            f"bucket={getattr(bucket, 'bucket_id', -1)} "
    ...
    *,
    ...
    try:
        ...
        if target_indices is not None and source_indices is not None:
            if int(target_indices.numel()) != int(source_indices.numel()):
                raise RuntimeError(
                    "[Matrix] mixed Matrix param gather index size mismatch "
                    f"bucket={getattr(bucket, 'bucket_id', -1)} "
                    f"target_numel={int(target_indices.numel())} "
                    f"source_numel={int(source_indices.numel())}"
                )
            source_params = _param_source_buffer(
                bucket,
                name="mixed_matrix_input",
                shape=(int(source_indices.numel()),),
                dtype=bucket.param_data.dtype,
                device=bucket.param_data.device,
            )
            torch.index_select(bucket.param_data.view(-1), 0, source_indices, out=source_params)
            gather_input.index_copy_(0, target_indices, source_params)
        fill_standard_param_gather(
            bucket,
            gather_input[matrix_size:gather_numel],
            standard_route,
        )
    except Exception:
        _release_param_gather_tensor(bucket, gather_input)
        raise
    return gather_input, prepared_entries


def _all_gather_mixed_params_(
    optimizer,
    bucket,
    *,
    async_op: bool,
    ...
    matrix_end = int(matrix_size)
    gather_numel = matrix_end + int(standard_numel)
    gathered_buffer = _acquire_param_gather_tensor(
        bucket,
        name="mixed_output",
        shape=(int(group_size) * gather_numel,),
        dtype=bucket.param_data.dtype,
        device=bucket.param_data.device,
        route_key=(int(group_size), int(matrix_size), int(standard_numel)),
    )
    ...
    def _restore_callback(_gather_input=gather_input, _gathered_buffer=gathered_buffer):
        try:
            gathered_view = _gathered_buffer.view(int(group_size), gather_numel)
            restore_bucket_param_data_(
                optimizer,
                bucket=bucket,
                prepared_entries=prepared_entries,
                gathered_buffer=gathered_view[:, :matrix_end],
                shard_group_size=int(group_size),
            )
            restore_standard_param_gather_(
                bucket,
                gathered_view[:, matrix_end:gather_numel],
                standard_route,
            )
        finally:
            _release_param_gather_tensor(bucket, _gather_input)
            _release_param_gather_tensor(bucket, _gathered_buffer)
...
def all_gather_bucket_params_(optimizer, bucket, async_op=False):
    """Gather all Matrix bucket params that do not follow the pure standard DO path."""
    matrix_layout = getattr(bucket, "matrix_layout", None)
    has_matrix = matrix_layout is not None and matrix_layout.has_params
    has_mixed_standard_params = has_matrix and bucket.has_standard_params

    prepared_matrix_entries = None
    if has_mixed_standard_params:
        ...
        if (
            ...
            standard_numel = int(standard_route.standard_numel)
            expected_gathered_numel = int(matrix_layout.gathered_numel)
            if expected_gathered_numel != matrix_size * int(standard_route.group_size):
                raise RuntimeError(
                    "[Matrix] combined mixed-bucket gather size invariant mismatch "
                    f"bucket={getattr(bucket, 'bucket_id', -1)} "
                    f"shard_size={matrix_size} group_size={int(standard_route.group_size)} "
                    f"expected_total={expected_gathered_numel}"
                )
            gather_input, prepared_entries = prepare_mixed_param_gather(
                optimizer,
                bucket,
                matrix_layout=matrix_layout,
                standard_route=standard_route,
            )
            return _all_gather_mixed_params_(
                optimizer,
                bucket,
                async_op=async_op,
        ...
        matrix_handle = all_gather_matrix_params_(
            optimizer,
            bucket,
            async_op=async_op,
            prepared_gather=prepared_matrix_entries,
        )
        if async_op and (standard_handle is not None or matrix_handle is not None):
            return BucketGatherHandle(standard_handle, matrix_handle)
        return None
