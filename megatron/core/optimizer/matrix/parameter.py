...
def all_gather_matrix_params_(
    ...
    if prepared_gather is None:
        prepared_buffer, prepared_entries = prepare_matrix_param_gather(optimizer, bucket)
    else:
...
def prepare_mixed_param_gather(
    ...
    *,
    ...
    payload_size = matrix_size + standard_numel
    ...
    try:
        ...
        fill_standard_param_gather(
            bucket,
            input_payload[matrix_size:payload_size],
            standard_route,
        )
    except Exception:
...
def _all_gather_mixed_payload_(
    ...
    *,
    ...
    payload_size = matrix_end + int(standard_numel)
    gathered_buffer = _acquire_param_gather_tensor(
        bucket,
        name="mixed_output",
    ...
    def _restore_callback(_input_payload=input_payload, _gathered_buffer=gathered_buffer):
        try:
            ...
            restore_standard_param_gather_(
                bucket,
                gathered_view[:, matrix_end:payload_size],
                standard_route,
            )
        finally:
...
def all_gather_bucket_params_(optimizer, bucket, async_op=False):
    ...
    if has_mixed_standard_params:
        ...
        prepared_matrix_entries = prepare_matrix_param_gather(optimizer, bucket)
