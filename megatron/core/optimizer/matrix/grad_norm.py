...
def _grad_sort_key(optimizer, shard_param) -> str:
    cache = getattr(optimizer, "_matrix_grad_norm_sort_key_cache", None)
    if cache is None:
        cache = {}
        setattr(optimizer, "_matrix_grad_norm_sort_key_cache", cache)
    cache_key = id(shard_param)
    cached = cache.get(cache_key)
    if cached is not None and cached[0] is shard_param:
        return cached[1]
    sort_key = repr(optimizer._shard_param_uid(shard_param))
    cache[cache_key] = (shard_param, sort_key)
    return sort_key
...
def _grad_sum_sq_fp64(tensor: torch.Tensor) -> torch.Tensor:
    ...
    if int(tensor.numel()) <= 0:
        return total_sq
    total_sq[0] = torch.linalg.vector_norm(tensor, ord=2, dtype=torch.float64).square()
    return total_sq
...
def matrix_replica_grads(
    ...
    matrix_params = sorted(
        matrix_params,
        key=lambda pair: _grad_sort_key(optimizer, pair[1]),
    )
    local_grads = [
    ...
    for indices in groups_by_dtype_device.values():
        ...
        for index in indices:
            ...
            next_cursor = cursor + int(grad.numel())
            reduced_view = flat_grad[cursor:next_cursor].view_as(grad)
            if dense_reuse[index]:
                grad.copy_(reduced_view)
            ...
            if count_matrix_grad:
                replica_grads[index] = reduced_view
            cursor = next_cursor
...
def contributes_matrix_grad(optimizer) -> bool:
    ...
    cache_key = (id(replica_group), id(stats_group))
    cache = getattr(optimizer, "_matrix_grad_contribution_cache", None)
    if cache is None:
        cache = {}
        setattr(optimizer, "_matrix_grad_contribution_cache", cache)
    cached = cache.get(cache_key)
    if (
        cached is not None
        and cached[0] is replica_group
        and cached[1] is stats_group
    ):
    ...
    if len(replica_ranks_in_stats) <= 1:
        ...
        contributes = int(dist.get_rank()) == int(replica_ranks_in_stats[0])
    cache[cache_key] = (replica_group, stats_group, contributes)
    return contributes
