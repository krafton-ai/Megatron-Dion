...
def get_qkvg_split_axis(param: torch.Tensor) -> int:
    """Return the fused QKVG split axis for a tagged parameter."""
    return get_split_axis_from_param(param, "qkvg_split_axis", default=0)
...
def get_qkvg_split_axis_from_dist_meta(dist_meta) -> int:
    """Return the QKVG split axis from distributed metadata."""
    return get_split_axis_from_dist_meta(dist_meta, "qkvg_split_axis", default=0)
...
def get_qkvg_split_axis_from_state(optimizer_state: Optional[dict]) -> int:
    """Return the QKVG split axis from optimizer state."""
    return get_split_axis_from_state(optimizer_state, "qkvg_split_axis", default=0)
...
def resolve_qkvg_split_axis(
    *,
    param: Optional[torch.Tensor] = None,
    optimizer_state: Optional[dict] = None,
    dist_meta=None,
) -> int:
    """Resolve the QKVG split axis from state, dist-meta, then param attrs."""
    return resolve_split_axis(
        attr="qkvg_split_axis",
        param=param,
...
def copy_qkvg_split_metadata(destination_tensor: torch.Tensor, source_tensor: torch.Tensor) -> None:
    ...
    copy_split_axis(destination_tensor, source_tensor, "qkvg_split_axis")
    ...
    *,
    ...
    if rows % total_per_group != 0:
        raise RuntimeError(
            ...
            f"split_shapes={split_shapes} "
            "matrix_split_qkvg requires an integral global grouped-QKVG row layout."
        )
...
def extract_qkvg_child(
    ...
    """Read one Q/Gate/K/V child from a fused QKVG tensor."""
    if tensor.ndim != 2:
        raise RuntimeError(
